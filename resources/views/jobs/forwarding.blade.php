...
#jobsTable tbody tr { border-bottom: 1px solid var(--border); cursor: pointer; transition: background .12s; }
#jobsTable tbody tr:hover { background: #f0f4ff; }
#jobsTable tbody tr.selected { background: #e8f0fe; }
#jobsTable tbody td { padding: 9px 12px; vertical-align: middle; }
</style>
@endpush
...
function loadJobs(contactId) {
    ...
    fetch(`/forwarding/jobs-for-contact?contact_id=${contactId}`)
        ...
        .then(jobs => {
            ...
            allJobs = jobs;
            if (!jobs.length) { noJobs.style.display = 'block'; return; }
