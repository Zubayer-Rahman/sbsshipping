...
<script>
    function generateBillItems(job, serviceCharge, percentage) {
        ...
        const isImportByAir = category.includes('import') &&
    ...
    function addServiceChargeRow(job, calculation) {
        ...
        // Skip if backend says so
        if (calculation.skip_agency_commission) {
            return;
        }
        ...
        let description = `Agency Commission - ${jobLabel}`;
        if (calculation.is_minimum_applied) {
            description += ` (${calculation.commission_type}: ৳${calculation.minimum_commission})`;
        } else {
            description += ` (${calculation.percentage}% of $${parseFloat(calculation.imp_exp_value).toLocaleString()})`;
        }
    ...
    `;
        ...
        showNotification(`✓ Added ৳${calculation.service_charge_amount.toFixed(2)} agency commission for ${jobLabel}`, 'success');
    }
    ...
    function fetchAndAddJobCharge(jobId) {
        ...
        fetch(`/bills/get-job-details/${jobId}`)
            ...
            .then(data => {
                ...
                // ✅ ALWAYS generate bill items for ALL job types
                generateBillItems(job, calc.service_charge_amount, calc.percentage);

                // ✅ Only add service charge if NOT import by air
                if (!calc.skip_agency_commission && calc.service_charge_amount > 0) {
                    addServiceChargeRow(job, calc);
                } else {
                    console.log('⏭️ Skipping agency commission - Import by Air or no charge');
                }

                addedJobsTracker.add(jobId);
                showNotification(`✓ Applied structure for Job ${job.job_id || job.job_no}`);
            })
            .catch(error => {
    ...
    function removeServiceChargeRow(rowId) {
        ...
        // ✅ Get jobId from the row's dataset BEFORE removing it
        const jobId = row.dataset.jobId;

        row.remove();
        console.log('🗑️ Removed service charge row:', rowId, 'for job:', jobId);

        if (typeof calculateExpenseTotal === 'function') calculateExpenseTotal();
        if (typeof renumberRows === 'function') renumberRows();

        // ✅ Uncheck the job if jobId exists
        if (jobId) {
            // Remove from tracker
            if (typeof addedJobsTracker !== 'undefined') {
            ...
            if (cb) {
                cb.checked = false;
                console.log('✅ Unchecked job checkbox:', jobId);
            }

            // Remove selected class from visual option
            const opt = document.querySelector(`.job-visual-option[data-id="${jobId}"]`);
            if (opt) {
                opt.classList.remove('selected');
                console.log('✅ Removed selected class from job option:', jobId);
            }

            // Sync job tags
            if (typeof syncJobTags === 'function') {
                syncJobTags();
            }
        }
    }
